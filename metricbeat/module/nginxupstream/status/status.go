...
const (
	...
	// defaultPath is the default path to the nginx_upstream_check_module endpoint on Nginx.
	defaultPath = "/nginx_upstream_status?format=json"
)

var (
	debugf = logp.MakeDebug("nginxupstream-status")
)

func init() {
	if err := mb.Registry.AddMetricSet("nginxupstream", "status", New); err != nil {
		panic(err)
	}
}

// MetricSet for fetching Nginx upstream status.
type MetricSet struct {
	mb.BaseMetricSet

	client *http.Client // HTTP client that is reused across requests.
	url    string       // Nginx upstream status endpoint URL.
...
func New(base mb.BaseMetricSet) (mb.MetricSet, error) {
	...
	debugf("nginxupstream-status URL=%s", u)
	return &MetricSet{
		BaseMetricSet: base,
		url:           u.String(),
